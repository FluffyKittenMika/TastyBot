...
{
    [Name("Cat")]
    public class CatImgModule : ModuleBase<SocketCommandContext>
    {
		...
		public async Task CatAsync(params string[] args)
		{
			...
			else
			{
				...
				else
				{
									...
									else
									{
										if (ColorPos == 0)
										{
											...
											else
											{
												...
												int VerySit = LenghtTot - 1;
												if (args.ElementAt(LenghtTot).ToLower() == "s" || args.ElementAt(LenghtTot).ToLower() == "size")
												{
													if (args.ElementAt(VerySit).ToLower() == "t" || args.ElementAt(VerySit).ToLower() == "txt")
													{
														await ReplyAsync("Splish splash, you forgot to add some text");
													}
													else
					...
					{
						await ReplyAsync("You messed up somehow");
					}
				}
